...
这个项目的核心是一个单入口的通用 Agent runtime。它通过 while-loop 驱动模型自主选择工具、执行任务、回灌结果，直到产出最终答案。公开用户心智统一为 Session：一个 Session 保存一整段可跨进程继续的对话，每条新消息创建一个 Turn；Checkpoint 只恢复尚未完成的 Turn。
...
Agent 会读取当前目录的 `.env`，但不会覆盖已经存在的宿主环境变量。当前默认模型是 Groq 上的 `groq_gpt_oss_120b`；将密钥放在 `.env` 即可：

```dotenv
GROQ_API_KEY=your-key
```

也可以完全走本地 Qwen3.5。`qwen3_5_9b_mlx_4bit` 会按 `configs/models.yaml` 检查 `127.0.0.1:8080/v1/models`，未启动时自动拉起 MLX OpenAI-compatible server：
...
`agent run` 会根据当前 workspace 中已安装、已启用的能力自动装配工具面。用户只需提交任务，不需要记工具名；CLI 会展示实际调用的工具和结果。普通工具审批显示有界参数，`run_command` 审批则完整显示命令、cwd、网络状态和执行模式：
...
长对话使用 Session。省略 `--session-id` 会创建新 Session；重新启动进程后传回同一个 Session ID，即可从持久化 canonical history 继续聊天并创建新 Turn：
...
当前基线来自 `3 trials × 5 cases × 2 models = 30` 条真实观察。成功率、工具选择、失败恢复、审批继续、重复失败控制和参数有效性下限均为 `1.0`；冗余失败重放上限为 `0.0`；每 case 平均工具调用和模型调用上限分别为 `2.0` 和 `3.0`。gate 退出码为：`0 = PASS`、`1 = QUALITY_FAIL`、`2 = INFRASTRUCTURE_INCONCLUSIVE / 配置错误`。原始观察和阈值保存在 `evals/model_quality/baseline_v1.json`，加载时会重新计算 trial 指标与阈值，篡改会 fail loud。

## News

- 2026-07-15：公开对话语义收敛为 Session → Turn → Checkpoint；支持跨进程续聊与多动作 Turn 恢复、canonical history、受限 `run_command` 沙箱、持久化 `update_plan`、Qwen3.5/Groq 真实模型质量门禁，以及可脱离源码 cwd 使用的 wheel 内置模型目录。
...
flowchart TB
    subgraph PUB["Public product surface"]
        direction LR
...
这个项目的主要业务场景是代码与文件任务、私有资料问答和可恢复的长对话。系统不是一组互相转发消息的角色 Agent，而是一个公开 product surface、一个 generic Agent kernel、一套 canonical ACI，以及一个按需启用的 RAG evidence pipeline。
...
`agent_runtime.Agent` 是 Python SDK facade，`agent run/chat/resume` 是同一能力的 CLI-first 表面。用户应使用的 identity 只有两个：`session_id` 表示整段对话，`turn_id` 表示一次用户消息触发的执行；兼容字段 `run_id / thread_id` 都归一到当前 Turn。所有新消息——包括跨进程继续聊天——都在原 Session 下创建新 Turn；完成的 Turn 永远不会被“恢复执行”。
...
Checkpoint 与 SessionStore 可以落在同一个 SQLite 文件中，但职责不同：Checkpoint 保存未完成 Turn 的执行快照；SessionStore 保存可长期继续的对话历史。Checkpoint 不能让已经退出的进程继续跑，也不是后台任务队列；真正离开终端后仍要持续执行的任务需要独立 worker/daemon，本项目当前不伪装提供这项能力。
...
模型目录的源码真相在 `configs/models.yaml`，构建时作为 package resource 写入 wheel。业务代码不直接写 provider、模型名、base URL 或 API key；加载优先级为 `RAG_AGENT_MODELS_PATH` YAML、`RAG_AGENT_MODELS` JSON、wheel 内置资源、源码目录配置。当前模型选择由 Model Control Plane 持有，不通过改 YAML 表示。CLI 可用：
...
PR 使用与 CI 相同的静态检查口径：import contracts 跑全量，Ruff 检查相对 base 的 Python 变更，mypy 检查变更的 `rag` source。全仓 Ruff/mypy 仍有历史基线噪音，不作为掩盖当前 diff 的理由：
