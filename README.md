...
Agent Layer：任务执行
  ResearchAgent、通用资产工具、TaskDAG、LangGraph、BudgetLedger、working memory
```
...
- 完成通用文件分析 Agent 主链路：RAG 定位资产 -> `asset_inspect` 读取结构 -> `asset_analyze` 执行只读分析 -> evidence/citation 回绑。
- 补齐更多底层资产 adapter：PDF 表格、PPT 表格/备注、Word 表格、图片 OCR 区域都转成统一 `Asset / Block / Locator / EvidenceRef / AnalysisResult`。
- 完善 LLM plan/evaluate provider 和 golden case 评测，重点覆盖制度问答、报销审批、销售日报 Excel 读数/汇总、歧义口径澄清。
