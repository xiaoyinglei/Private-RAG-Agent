...
- `AgentState` 使用 TypedDict，配合 LangGraph reducer 合并 messages、evidence、citations、tool results、structured observations 和 goal satisfaction fields。
- `GoalSpec / StructuredObservation / StateReducer / SatisfactionChecker` 构成 Goal Satisfaction Runtime。它表达“什么叫完成”、把工具输出归约成可检查状态，并判断是否继续、暂停、调用 LLM 或最终回答。
...
AgentRunRequest
  ...
  -> Controller Loop
     ...
     -> SatisfactionChecker
        -> done: Finalizer
        -> open gaps: ToolDecisionProvider selects next tool
        -> stuck or unsafe: pause / clarification
  -> AgentRunResult
```

旧 `route/plan/evaluate` 节点不再属于主控制流。`retrieval_hint` 只生成检索元数据，`llm_decide` 选择工具，`SatisfactionChecker` 判断完成状态，`synthesize` 只承担 Finalizer 表达职责。任务复杂度由 `open_gaps`、conflicts、evidence completeness、computation completeness 和 progress state 自然产生，不由 `simple/complex/table/word` 这类入口标签预先决定。
