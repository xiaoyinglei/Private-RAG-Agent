...
Agent 层采用 Tool-Centric + LangGraph 设计。

### 已实现

- `ToolSpec / ToolPermissions / ToolResult / ToolError`：工具输入、输出、权限、错误和预算契约。
- `ToolRegistry`：注册工具与 callable runner，执行 Pydantic 输入/输出校验。
- `AgentRunConfig / RuntimeRegistry / BudgetLedger`：区分可序列化运行配置和不可序列化 runtime handles。
- `AgentDefinition / ModelPolicy / ToolPolicy`：定义 Agent 类型、系统提示、工具 allowlist、模型策略和工具策略。
- `AgentState`：TypedDict 状态与 reducer，合并 messages、evidence、citations、tool results、subtask results。
- `TaskDAG`：子任务、依赖边、环检测、ready subtask 调度和失败阻断。
- `AgentRegistry / AgentToolSpec / AgentAsToolRunner`：注册 Agent，并将子 Agent 作为工具执行。
- LangGraph base graph：`route -> plan -> execute -> observe -> evaluate -> synthesize`。
- `execute_subagent`：支持 `Send()` 并行扇出，子任务完成后回到 evaluate 调度下一批。
- Working memory Phase A：working summary、extracted facts、context injection。
- 内置 ResearchAgent：提供 research AgentDefinition 和 service factory。

### 失败语义

Agent 层对失败保持显式状态：

- 工具未注册、未实现、参数非法、输出非法、预算不足、timeout、runner 异常都会生成结构化 `ToolResult(status="error")`。
- 子 Agent runner 抛异常、子 Agent 返回 failed、runtime handles 缺失、plan provider 缺失都会写入明确失败状态。
- TaskDAG 中失败子任务会成为 terminal，但不会进入 successful，因此不会解锁依赖它的下游任务。
- synthesize 阶段会把 tool/subtask/graph failure reason 反映到最终结果。

### 路线图

- 生产级 LLM plan/evaluate provider 和 prompt/eval。
- LangGraph `interrupt()`、checkpointer、`Command(resume=...)` 恢复链路。
- Orchestrator、CompareAgent、FactCheckAgent、SynthesizeAgent 等更多内置 AgentDefinition。
- RAG tool runner 与 L3-L6 runtime 的完整绑定。
- Agent CLI/API 与 golden case 评测。
