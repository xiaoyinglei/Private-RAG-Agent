...
| MCP 工具接入 | MCP Adapter 收敛，外部工具通过统一 Catalog 发现 | `rag/agent/capabilities/` |
...
```text
Agent Runtime：任务执行
  generic AgentLoop、GENERIC_SYSTEM_PROMPT、ToolCatalog、DeferredToolStore、ToolExecutionService、Workspace、Memory、Checkpoint
...
- **完善 MCP / Skills adapter**：外部 MCP 工具和 Skills 进入同一个 ToolCatalog 的适配层，但不增加新的根 Agent 角色。
